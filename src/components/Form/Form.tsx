...
import { addContact, updateContact } from "../../store/contactsReducer";
...
export const Form = ({ currentId, setCurrentId }: FormPropsType) => {
  ...
  const contact = useAppSelector((state) =>
    currentId
      ? state.contacts.allContacts.find((contact) => contact._id === currentId)
      : null
  );
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (currentId) {
      dispatch(updateContact({ id: currentId, contact: contactData }));
    } else {
    ...
    <Paper className={classes.paper} elevation={6}>
      ...
      >
        <Typography variant={"h6"}>
          {currentId ? "Editing" : "Creating"} a contact
        </Typography>
        <TextField
          name={"firstName"}
          variant={"outlined"}
        ...
        <TextField
          ...
          onChange={(e) =>
            setContactData({ ...contactData, adress: e.target.value })
          }
        />
        <div className={classes.fileInput}>
          <label>
            <input type="file" onChange={uploadHandler} multiple={false} />
          </label>
        </div>
        <Button
